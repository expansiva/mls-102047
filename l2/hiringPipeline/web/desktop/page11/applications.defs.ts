export const descriptions = [
  {
    ...
    "kind": "list",
    "description": "Shows applications by candidate, job position, and stage so a recruiter can locate work to review. It supports retrieving the application collection with the declared application lookup values and page value. It communicates loading while results are requested, an empty result when no applications match, and an error when retrieval fails. Results and retrieval controls must be keyboard usable, have clear accessible names, and announce status updates.",
    "contentRef": "base",
    "capabilityRefs": [
      "listApplication",
    ...
    "moleculeRecommendations": [
      {
        ...
        "reason": "The declared application query returns a collection that recruiters need to scan and retrieve by the available lookup values."
      },
      {
  ...
  },
  {
    ...
    "kind": "detail",
    "description": "Presents the selected application with its candidate, job position, and current stage so the recruiter can assess screening information, qualifications, position context, and remaining hiring need before deciding. It retrieves related application, candidate, and job-position information; it communicates loading and retrieval failures, and states when requested information is unavailable. Related information and current stage must have accessible labels and be readable in a logical order.",
    "contentRef": "base",
    "capabilityRefs": [
      "listApplication",
      "listCandidate",
  ...
  },
  {
    ...
    "kind": "form",
    "description": "Lets a recruiter start screening by choosing a candidate and an open job position, entering the required application status and details, and optionally entering a rejection reason. It retrieves candidate and job-position choices, validates required values before creating the application, indicates submission in progress, confirms success, and explains submission errors without discarding entered values. Every input needs an associated accessible label and required-state announcement, while selection and submission remain keyboard operable.",
...
export const pipeline = [
  {
    "id": "applications__desktop__page11",
    "type": "l2_page",
    "defPath": "l2/hiringPipeline/web/desktop/page11/applications.defs.ts",
    "outputPath": "l2/hiringPipeline/web/desktop/page11/applications.ts",
    "dependsFiles": [
